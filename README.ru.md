...
| Параметр                  | Тип       | По умолчанию                  | Описание                                                                                  |
| ------------------------- | --------- | ----------------------------- | ----------------------------------------------------------------------------------------- |
| `restoreMissingToCache`   | `boolean` | `true`                        | При промахе кеша: запрос в сеть + в фоне загрузить файл в кеш.                             |
...
3. При отсутствии файла и `restoreMissingToCache: true` текущий запрос идёт в сеть (отменяемый range). В фоне полный файл загружается в кеш для следующих запросов **только если его URL входит в список `assets`** (если `assets` задан; иначе восстановление выполняется для любого URL).
