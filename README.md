...
3. If the file is missing and `restoreMissingToCache` is true: if a restore is already in progress, waits for it (up to `restoreWaitTimeout`) and retries from cache; otherwise returns `undefined` and starts a background fetch to restore the file for future requests.
